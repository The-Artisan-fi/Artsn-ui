...
const handler = async (req: NextRequest) => {
  ...
  try {
    const scriptUrl =
      'https://script.google.com/macros/s/AKfycbyMAq0FYFRFbDp6l6fOEKdDyxmrjtRCuLQm_j-QbcI4yl8fqmDk6zMkMiN5QTfEGTAu6w/exec'
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to add to waitlist: ${response.status} ${response.statusText}`
      )
    }
